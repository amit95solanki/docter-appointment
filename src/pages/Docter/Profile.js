...
const Profile = () => {
  ...
  const handleFinish = async (values) => {
    try {
      ...
      const res = await axios.post(
        'https://docter-appointment-amit-backend.vercel.app/api/v1/doctor/updateProfile',
        {
          ...values,
          userId: user._id,
  ...
  const getDoctorInfo = async () => {
    try {
      const res = await axios.post(
        'http://localhost:9090/api/v1/doctor/getDoctorInfo',
        { userId: params.id },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          ...
          <Row gutter={20}>
            ...
            <Col xs={24} md={24} lg={8}>
              <Form.Item
                ...
                name="timings"
                format="HH:mm"
                value={[moment(doctor.timings[0], 'HH:mm'), moment(doctor.timings[1], 'HH:mm')]}
                required
              >
